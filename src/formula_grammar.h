 ...
 * Revision 1.3  2013-12-17 08:55:58  graziano
...
struct formula_grammar : public grammar<formula_grammar>
{
    ...
    static const int func_tangoID = 23;
    static const int val_sleepID = 24;
    static const int func_gotoID = 25;
...
//            	=	repeat_p(3)[(+symbol) >> ch_p('/')] >> (+symbol) 
            ...
            index
            	...
            	//= 	reduced_node_d[ch_p('[') >> uint_p >> ch_p(']')]
            	;
            //------------------------------FORMULA--------------------------------------	   
			val_r
#if BOOST_VERSION  < 103600              
...
//            	=	repeat_p(3)[(+symbol) >> ch_p('/')] >> (+symbol)
            ...
            function_tango
            	=	( 
            		root_node_d[str_p("read")] >> (inner_node_d[ch_p('(') >> name >> !(index) >> ')'])
//            	|	root_node_d[str_p("write")] >> (inner_node_d[ch_p('(') >> name >> !(index) >> ')']) >> '=' >> (val_h | val_r | val_st | val_string | (str_p("read") >> (inner_node_d[ch_p('(') >> name >> !(index) >> ')'])))
                |	root_node_d[str_p("write")] >> (inner_node_d[ch_p('(') >> name >> !(index) >> ')']) >> '=' >> cond_expr
            	|	root_node_d[str_p("command")] >> (inner_node_d[ch_p('(') >> name >> ')'])  >> '=' >> (val_h | val_r | val_st | val_string)
            	|	root_node_d[str_p("command")] >> (inner_node_d[ch_p('(') >> name >> ')'])
            	)
            	;
            val_sleep
            //	=	root_node_d[str_p("sleep")] >> (ch_p('(') >> val_r >> ')')
        		=	root_node_d[str_p("sleep")] >> (inner_node_d[ch_p('(') >> val_r >> ')'])
            	;
            function_goto
            //	 = root_node_d[str_p("goto")] >> (ch_p('(') >> val_r >> ')')
            	 = root_node_d[str_p("goto")] >> (inner_node_d[ch_p('(') >> val_r >> ')'])
            	 ;
           ...
           // top = non_empty_expression;
            ...
            expr_atom
                =	//val_h | val_r
					event_
					| val_h | val_r | val_st | val_string
					| val_sleep | function_goto
		...
		rule<ScannerT, parser_context<>, parser_tag<func_tangoID> > function_tango;
		rule<ScannerT, parser_context<>, parser_tag<val_sleepID> > val_sleep;
		...
		rule<ScannerT, parser_context<>, parser_tag<func_gotoID> > function_goto;
